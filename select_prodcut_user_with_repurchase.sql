...
-- 여기서 조인할 때의 중요한 것은 자기 자신을 조인하면서 생기는 중복 레코드이다. 
-- 예를 들어, 첫번째 열에서 세번째 열과 조인이 일어나면 세번째 열 입장에서도 첫번째 열과 조인이 일어난다.  
-- 따라서 DISTINCT가 필수적이다.

SELECT DISTINCT F.USER_ID, F.PRODUCT_ID 
FROM ONLINE_SALE AS F INNER JOIN ONLINE_SALE AS R
ON F.USER_ID = R.USER_ID AND F.PRODUCT_ID = R.PRODUCT_ID
AND F.SALES_DATE <> R.SALES_DATE
ORDER BY F.USER_ID, R.PRODUCT_ID DESC;